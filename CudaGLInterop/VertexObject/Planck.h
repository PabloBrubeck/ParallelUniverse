...
uchar4 XYZ2RGB(float3 r, float3 g, float3 b, float3 color){
	...
	float denominator=(r.x*g.y-g.x*r.y)*b.z+(b.x*r.y-r.x*b.y)*g.z
		+(g.x*b.y-b.x*g.y)*r.z;
    float red=((color.x*g.y-g.x*color.y)*b.z+(g.x*b.y-b.x*g.y)*color.z
        +(b.x*color.y-color.x*b.y)*g.z)/denominator;
    float green=((r.x*color.y-color.x*r.y)*b.z+(b.x*r.y-r.x*b.y)*color.z
        +(color.x*b.y-b.x*color.y)*r.z)/denominator;
    float blue=((r.x*g.y-g.x*r.y)*color.z+(color.x*r.y-r.x*color.y)*g.z
        +(g.x*color.y-color.x*g.y)*r.z)/denominator;

    red=  __max(0, __min(1, red  ));
	green=__max(0, __min(1, green));
	blue= __max(0, __min(1, blue ));
	...
	return make_uchar4(red, green, blue, 255);
}
__device__
uchar4 planckColor(float temperature){
	float3 red   = make_float3(0.64f, 0.33f, 0.01f);
	float3 green = make_float3(0.29f, 0.60f, 0.11f);
	float3 blue  = make_float3(0.15f, 0.06f, 0.79f);
